...
const ALLOW_PUBLIC_STREAM = process.env.ALLOW_PUBLIC_STREAM === "true";
...
const MAX_REPROMPTS = parseInt(process.env.MAX_REPROMPTS || "3", 10);            // max "still there?" prompts per call
const IDLE_CLOSE_MS = parseInt(process.env.IDLE_CLOSE_MS || "30000", 10);       // close WS if no media frames for this long
const MAX_TTS_CALLS_PER_SESSION = parseInt(process.env.MAX_TTS_CALLS || "25", 10); // absolute cap on TTS calls per WS session
const MAX_SESSION_MS = parseInt(process.env.MAX_SESSION_MS || String(10 * 60 * 1000), 10); // hard cap per WS session (default 10 min)
...
const resetNoInputTimer = () => {
  if (ws._closed || ws.readyState !== ws.OPEN) return;
  ...
  noInputTimer = setTimeout(async () => {
    ...
    // Hard session cap
    if (Date.now() - ws._sessionStartedAt > MAX_SESSION_MS) {
      console.warn("[SAFETY] Max session duration reached, closing socket");
      ws._closed = true;
      try { ws.close(); } catch {}
      return;
    }

    // Don't reprompt before stream has actually started
    if (!ws._started) return;

    // If the stream is idle, close it instead of repeatedly spending money
    if (ws._lastMediaAt && (Date.now() - ws._lastMediaAt > IDLE_CLOSE_MS)) {
      console.warn("[SAFETY] Idle stream detected, closing socket");
      ws._closed = true;
      try { ws.close(); } catch {}
      return;
    }
    ...
    ws._repromptCount += 1;
    if (ws._repromptCount > MAX_REPROMPTS) {
      console.warn("[SAFETY] Max reprompts reached, closing socket");
      ws._closed = true;
      try { ws.close(); } catch {}
      return;
    }

    // Absolute TTS cap per session
    ws._ttsCalls += 1;
    if (ws._ttsCalls > MAX_TTS_CALLS_PER_SESSION) {
      console.warn("[SAFETY] Max TTS calls reached, closing socket");
      ws._closed = true;
      try { ws.close(); } catch {}
      return;
    }
    ...
    try {
      ...
      // If we can't speak, we should stop. Retrying every 7s is a runaway-cost bug.
      console.error("[TTS] reprompt failed; closing socket:", e.message);
      ws._closed = true;
      try { ws.close(); } catch {}
      return;
    } finally {
    ...
    // Only reschedule after a successful reprompt
    resetNoInputTimer();
...
// Reset reprompt counter as soon as we get real user speech
ws._repromptCount = 0;

// De-dupe near-identical finals (prevents "partial promote" + "true final" double replies)
const norm = String(finalText || "").trim().toLowerCase().replace(/\s+/g, " ");
if (norm.length === 0) return;
if (ws._lastFinalNorm && ws._lastFinalAt && (Date.now() - ws._lastFinalAt < 2500)) {
  // simple similarity check: one contains the other
  if (ws._lastFinalNorm.includes(norm) || norm.includes(ws._lastFinalNorm)) {
    console.log("[ASR] Dropping duplicate/near-duplicate final");
    ws._lastFinalNorm = norm;
    ws._lastFinalAt = Date.now();
    return;
  }
}
ws._lastFinalNorm = norm;
...
// Absolute TTS cap per session (guard here too because main replies spend money)
if (ws._ttsCalls >= MAX_TTS_CALLS_PER_SESSION) {
  console.warn("[SAFETY] TTS cap reached, closing socket");
  ws._closed = true;
  try { ws.close(); } catch {}
  return;
}
  ...
  ws.on("message", async (data) => {
    ...
    if (msg.event === "start") {
      ...
      ws._started = true;
      ws._lastMediaAt = Date.now();
      ws._repromptCount = 0;
      console.log(`[WS] START callSid=${safeLog(msg.start?.callSid || "")}`);

      // Idle watchdog (belt + suspenders)
      if (!idleInterval) {
        idleInterval = setInterval(() => {
          if (ws._closed || ws.readyState !== ws.OPEN) return;
          if (Date.now() - ws._sessionStartedAt > MAX_SESSION_MS) {
            console.warn("[SAFETY] Max session duration reached (watchdog), closing socket");
            ws._closed = true;
            try { ws.close(); } catch {}
            return;
          }
          if (ws._lastMediaAt && (Date.now() - ws._lastMediaAt > IDLE_CLOSE_MS)) {
            console.warn("[SAFETY] Idle stream detected (watchdog), closing socket");
            ws._closed = true;
            try { ws.close(); } catch {}
          }
        }, 1000);
      ...
      try {
        ...
        ws._ttsCalls += 1;
        if (ws._ttsCalls > MAX_TTS_CALLS_PER_SESSION) {
          console.warn("[SAFETY] Max TTS calls reached during greeting, closing socket");
          ws._closed = true;
          try { ws.close(); } catch {}
          return;
        }
        ...
        console.error("[TTS] greeting failed; closing socket:", e.message);
        ws._closed = true;
        try { ws.close(); } catch {}
        return;
      }
  ...
  ws.on("close", () => {
    ...
    if (idleInterval) { clearInterval(idleInterval); idleInterval = null; }
    if (ws._dgConnection && ws._dgConnection.readyState === ws._dgConnection.OPEN) {
      try { ws._dgConnection.close(); } catch {}
    }
  });
...
app.get("/debug/say", async (req, res) => {
  // NEVER leave this endpoint open in production.
  if (process.env.ENABLE_DEBUG_SAY !== "true") return res.status(404).send("Not found");

  const token = (req.query.token || "").toString();
  if (!AGENT_TOKEN || token !== AGENT_TOKEN) return res.status(401).send("Unauthorized");
...
server.on("upgrade", (req, socket, head) => {
  try {
    ...
    // Authentication: require a token unless you explicitly allow public access.
    // Leaving /stream public is a common runaway-cost footgun.
    if (!ALLOW_PUBLIC_STREAM) {
      if (!AGENT_TOKEN) {
        console.log("❌ AGENT_TOKEN is not set; refusing public /stream");
